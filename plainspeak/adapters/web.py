...
The "Simplified Text" it serves is the governed pipeline's presentation — the
same operation as `plainspeak present`: SAFE changes only, every one through the
integrity firewall. Review-required style suggestions are not applied here.
...
#: choice made here, at the edge, and reported in every response — the engine
#: itself has no default. The SAFE changes that make up the presented text do
#: not depend on the profile; the profile governs style observations.
WEB_PROFILE = "natural"
  ...
  <div class="results" id="results" aria-label="Analysis results">
    ...
    <div class="tab-panel active" id="panel-simplified" role="tabpanel">
      <div class="warning-note">
        <strong>Governed presentation.</strong> Only changes PlainSpeak classifies as SAFE are applied, and each one
        passed the integrity firewall, which refuses anything touching numbers, dates, amounts, identifiers,
        negation or obligations. Changed words are <mark>highlighted</mark>. Style suggestions that need a person
        are not applied here — review them in the PlainSpeak desktop application.
      </div>
...
def create_app():
    ...
    def api_analyze():
        ...
        Expects JSON body: {"text": "...", "no_simplify": false, "profile": "natural"}
        Returns full readability scores + simplification results. The
        simplified text is the governed presentation: SAFE changes only.
        """
        try:
        ...
        if not no_simplify:
            ...
            profile_id = (body.get("profile") if body else None) or WEB_PROFILE
            try:
                presented = present_text(text, profile_id)
            except PresentError as error:
                return jsonify({"error": error.message, "code": error.code}), 400
            simplified_text = presented.marked_text()
            result_dict["presentation"] = {
                "profile": presented.bundle.profile_id,
                "output_sha256": presented.preview.output_hash,
                "applied": len(presented.applied),
                "review": len(presented.review),
                "refused": len(presented.refused),
            }
